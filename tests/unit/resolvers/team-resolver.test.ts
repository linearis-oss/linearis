...
function mockSdkClient(
  ...callResults: Array<{ nodes: Array<{ id: string; key?: string; name?: string }> }>
) {
  const teams = vi.fn();
  callResults.forEach((result) => teams.mockResolvedValueOnce(result));
...
describe("resolveTeamId", () => {
  ...
  it("throws when team not found by key or name", async () => {
    ...
    await expect(resolveTeamId(client, "NOPE")).rejects.toThrow('Team "NOPE" not found');
  });
});
