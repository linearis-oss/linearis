...
| `projects` | 🟡 | List, read, create, update, delete (trash) and unarchive (restore); assign project labels by name (`--labels`, `--label-mode`, `--clear-labels`); status updates (list, read, create, edit, archive/unarchive, remind); discussions | Project-label CRUD, project relations, project status administration, Slack channel creation |
