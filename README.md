...
| `projects` | 🟡 | List, read, create, update, delete (trash) and unarchive (restore); assign project labels by name (`--labels`, `--label-mode`, `--clear-labels`); discussions | Project updates (status posts), project-label CRUD, project relations, project status administration, Slack channel creation |
